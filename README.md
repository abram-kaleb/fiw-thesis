...
```text
.devcontainer/                     Definition des Development Containers
    devcontainer.json              Konfiguration der kontainerisierten Entwicklungsumgebung
    Dockerfile                     Dockerfile für die kontainerisierte Entwicklungsumgebung
.vscode/                           Einstellungen und Erweiterungen für VSCode
    extensions.json                Empfohlene Erweiterungen
    settings.json                  VSCode Einstellungen speziell für dieses Projekt
...
fiwthesis.cls                      Dokumentenklasse
...
## TL;DR – der einfache Weg

1. [VSCode](https://code.visualstudio.com) installieren
2. Docker Desktop installieren (für [Mac](https://docs.docker.com/desktop/install/mac-install/), [Windows](https://docs.docker.com/desktop/install/windows-install/), [Linux](https://docs.docker.com/desktop/install/linux/))
3. Dieses Projekt in VSCode öffnen
4. VSCode wird vorschlagen die in `.vscode/extensions.json` genannten Erweiterungen zu installieren, diese Aktion bestätigen.
5. VSCode wird fragen ob der vorhandene DevContainer genutzt werden soll, dies ebenfalls bestätigen.
6. Im DevContainer selbst wird ebenfalls nach den Erweiterungen gefragt. Da diese im DevContainer andere sind als in Schritt 4, abermals bestätigen.
7. Nun kann über das Terminal der Bauprozess angestoßen werden.
    - Dazu über den Reiter 'Terminal/Neues Terminal' eine neue Terminal-Instanz starten. Diese sollte sich am unteren Fensterrand öffnen.
    - Durch die Eingabe von `make all` wird nun das Dokument gebaut
    - Durch die Eingabe von `make clean` werden alle Daten des Bauprozesses (auch die PDF) gelöscht.
...
Zum Erstellen Ihrer Abschlussarbeit im PDF-Format benötigen Sie:

- eine aktuelle LaTeX-Distribution
  ([TeX Live](https://www.tug.org/texlive/) oder [MiKTeX](https://miktex.org/))
- den LaTeX-Compiler `lualatex` (üblicherweise in den Distributionen enthalten)
- [Perl](https://www.perl.org/get.html) und die Hilfsprogramme [`makeglossaries`](https://ctan.org/tex-archive/macros/latex/contrib/glossaries) und [`biber`](https://biblatex-biber.sourceforge.net/) zum Erstellen des Glossars sowie des Abkürzungs- und Symbolverzeichnisses und der Bibliographie (sollten ebenfalls in der Distribution enthalten sein)

Aufgrund der Pakete _fontspec_ und _selnolig_ muss zwangsläufig \textbf{LuaLaTeX} als Compiler verwendet werden.
...
Präsentationsfolien im Design der Hochschule Wismar mit [LaTeX Beamer](https://www.overleaf.com/learn/latex/Beamer).

[https://git.fiw.hs-wismar.de/HSW-Vorlagen/beamertheme-wismar](https://git.fiw.hs-wismar.de/HSW-Vorlagen/beamertheme-wismar)
